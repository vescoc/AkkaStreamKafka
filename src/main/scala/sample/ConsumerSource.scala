...
class ConsumerSource[K, V](
  _consumer: => Consumer[K, V],
  topics: Seq[String],
  timeout: FiniteDuration,
  sleepTime: FiniteDuration
) extends GraphStage[SourceShape[Record[K, V]]] {
...
object ConsumerSource {
  ...
  ): Source[Record[K, V], NotUsed] =
    Source.fromGraph(new ConsumerSource(consumer, topics, timeout, sleepTime))
